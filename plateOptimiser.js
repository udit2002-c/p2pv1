...
function submitPlateOptimiserForm() {
  ...
  const platesShort = parseInt(
    document.getElementById("platesShort").value,
    10
  );
  const platesExcess = parseInt(
    document.getElementById("platesExcess").value,
    10
  );
  ...
  fetch("/api/plate-optimizer", {
    ...
    .then((responseData) => {
      ...
      ).innerHTML = `Weekly Report ${responseData.weeklyAverages.averagePlatesShort.toFixed(
        2
      )} plates short, ${responseData.weeklyAverages.averagePlatesExcess.toFixed(
        2
      )} plates in excess`;
    })
    .catch((error) => {
